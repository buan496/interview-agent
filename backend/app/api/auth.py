...
from app.rate_limit import RateLimitBackendUnavailable, RateLimitExceeded, check_auth_rate_limits, rate_limit_http_exception
from app.rbac import ROLE_USER, can_manage_content, can_manage_question_bank, admin_access_decision, get_user_role
...
async def require_rubric_operator_or_admin(
    current_user: User = Depends(get_current_user),
    request: Request = None,
    db: AsyncSession = Depends(get_db),
) -> User:
    settings = get_settings()
    user_role = get_user_role(current_user)
    allowed = can_manage_content(current_user, settings)
    access_source = "role" if user_role in {"admin", "content_operator"} else "admin_phone_fallback"
    audit_metadata = {
        "method": request.method if request else None,
        "path": request.url.path if request else None,
    ...
    if not allowed:
        if isinstance(db, AsyncSession):
            await record_audit_event(
                db,
                action="rubric_denied",
                status="denied",
                actor=current_user,
                actor_role=user_role,
                resource_type="rubric",
                request=request,
                reason="rubric_rbac_denied",
                metadata=audit_metadata,
            )
        raise HTTPException(status_code=403, detail="Rubric operator privileges required")
    return current_user
